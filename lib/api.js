...
SpreadSheetApi.prototype.request = function(opts, callback) {
  ...
  return this.oAuth2.request(opts, function(err, body, res) {
    if (err) {
      if (res && res.body &&
          (res.body.message === 'No access or refresh token is set.' ||
          res.body.error_description === 'Missing required parameter: refresh_token')) {
        err.status = 401;
      } else {
    ...
    logger.debug(res.req.method, res.req.path);
    var _err;
