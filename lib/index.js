...
/**
 * to spreadsheet format date string
 * @param {number|string} value
 * @returns {string} - YYYY/MM/DD hh:mm:ss
 */
function toSheetDateString(value) {
...
Spread2Json.prototype.toCells = function(worksheetData) {
        ...
        if (hasArray) {
          ...
          _.forEach(arr, function(d, j) {
            ...
            if (value !== undefined) {
              cells.push({
                col: i + 1,
                row: row + j,
                value: type === 'date' ? toSheetDateString(value) : value
              });
          ...
          if (value !== undefined) {
            cells.push({
              col: i + 1,
              row: row,
              value: isSplitArray ? value.join() : (type === 'date' ? toSheetDateString(value) : value)
            });
          }
        }
      } catch (e) {
  ...
  _.forEach(worksheetData.datas, function(data) {
    if (opts.type === 'array') {
      var __ref = data[opts.ref_key || self.opts.ref_key];
      var datas = _.get(data, opts.key);
      _.forEach(datas, function(data) {
        data = _.assign({ __ref: __ref }, data);
        addCell(data);
        row = maxRow = maxRow + 1;
      });
    } else {
